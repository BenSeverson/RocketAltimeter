   	...
   	for(r=0;r<16;r++)
	{
		for(g=0;g<16;g++)
		{
			...
			for(b=0;b<16;b++)
			{
				...
				//msg[0]=(b<<4)|b;
				//msg[1]=b;
				//i2c_smbus_write_block_data(file, 0x10, 2, msg);
				//i2c_smbus_write_byte_data(file, 0x10, b| (b<<4));
				i2c_smbus_write_byte_data(file, 0x11, b);
				printf("R: 0x%02X\tG: 0x%02X\tB:%02X\n",r,g,b);
				//printf("turning on port %d at addr %d...\n",b,b/2+0x10);
				//if(b%2==0)
				//	r = 0xF0;
				//else
				//	r = 0x0F;
				//i2c_smbus_write_byte_data(file, 0x10 + b/2, r);
				usleep(5000);
			}
		}
	}
...
while(1)
{
	for(i=0;i<16;i++)
	{
		i2c_smbus_write_byte_data(file, 0x10, i<<4);
		i2c_smbus_write_byte_data(file, 0x11, i);
		printf("R: 0x%X\tG: 0x%X\tB:%X\n",i,0,i);
		usleep(50000);
	}
	for(i=1;i<16;i++)
	{
		i2c_smbus_write_byte_data(file, 0x10, (0x0F-i)<<4);
		printf("R: 0x%X\tG: 0x%X\tB:%X\n",0x0F-i,0,0x0F);
		usleep(50000);
	}
	for(i=1;i<16;i++)
	{
		i2c_smbus_write_byte_data(file, 0x10, i);
		printf("R: 0x%X\tG: 0x%X\tB:%X\n",0,i,0x0F);
		usleep(50000);
	}
	for(i=1;i<16;i++)
	{
		i2c_smbus_write_byte_data(file, 0x11, 0x0F-i);
		printf("R: 0x%X\tG: 0x%X\tB:%X\n",0,0x0F,0x0F-i);
		usleep(50000);
	}
	for(i=1;i<16;i++)
	{
		i2c_smbus_write_byte_data(file, 0x10, (i<<4) | 0x0F);
		printf("R: 0x%X\tG: 0x%X\tB:%X\n",i,0x0F,0);
		usleep(50000);
	}
	for(i=1;i<16;i++)
	{
		i2c_smbus_write_byte_data(file, 0x10, 0xF0 | (0x0F-i));
		printf("R: 0x%X\tG: 0x%X\tB:%X\n",0x0F,0x0F-i,0);
		usleep(50000);
	}
	for(i=1;i<16;i++)
