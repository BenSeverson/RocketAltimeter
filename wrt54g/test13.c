/* R-G-B travel across then down single square at a time */
...
int main(int argc, char *argv[]) {
	...
	while(1)
	{
		for(j=0;j<9;j++)
		{
			intensity[j/3][j%3].red = 15;
			write_colors(file,intensity);
			intensity[j/3][j%3].red = 0;
			//usleep(50000);
		}
		for(j=0;j<9;j++)
		{
			intensity[j/3][j%3].green = 15;
			write_colors(file,intensity);
			intensity[j/3][j%3].green = 0;
			//usleep(50000);
		}
		for(j=0;j<9;j++)
		{
			intensity[j/3][j%3].blue = 15;
			write_colors(file,intensity);
			intensity[j/3][j%3].blue = 0;
			//usleep(50000);
		}
	}
