...
void I2C_setup()
{
  ...
  PIR1bits.SSPIF = 0;
  SSPADD = 119;					  //set to 100kHz
  SSPSTAT = 0b10000000;                // power on state 
  SSPCON2 = 0x00;                 // power on state
  SSPCON1 = 0b00101000;                 // power on state
...
unsigned char I2C_EEwrite(unsigned char data, unsigned int address, unsigned char block)
{
	char no_ack = 0;
	char x = 9;
	block &= 1;									//only need last bit
	
	I2C_idle();									//nothing else should be happening at this time
	I2C_start();
	while ( SSPCON2bits.SEN ){}					//wait for start to finish
	
	x = I2C_write(0xA6);		//last bit=0 for write
	if (SSPCON2bits.ACKSTAT){no_ack = 1;}		//test for ack
	x = I2C_write((unsigned char)(address >> 8));	//write high address byte
	if (SSPCON2bits.ACKSTAT){no_ack = 1;}		//test for ack
	x = I2C_write((unsigned char)(address));		//write low address byte
	if (SSPCON2bits.ACKSTAT){no_ack = 1;}		//test for ack
	x = I2C_write(data);							//write to address
	if (SSPCON2bits.ACKSTAT){no_ack = 1;}		//no ack here?	
	I2C_stop();									//all done
	
	while ( SSPCON2bits.PEN );					//wait for stop bit to finish
	if ( PIR2bits.BCLIF ){no_ack = 1;}			//test for bus collision
	return no_ack;
}


//random byte read
unsigned char I2C_EEread(unsigned int address, unsigned char block)
{
	char no_ack = 0;
	unsigned char data;
	block &= 1;									//only need last bit
	...
	I2C_write(0xA6);		//last bit=0 for write
	if (SSPCON2bits.ACKSTAT){no_ack = 1;}		//test for ack
	I2C_write((unsigned char)(address >> 8));	//write high address byte
	if (SSPCON2bits.ACKSTAT){no_ack = 1;}		//test for ack
	I2C_write((unsigned char)(address));		//write low address byte
	if (SSPCON2bits.ACKSTAT){no_ack = 1;}		//test for ack
	
	I2C_restart();
	while(SSPCON2bits.RSEN){}
	
	I2C_write(0xA7);		//last bit=1 for read
	if (SSPCON2bits.ACKSTAT){no_ack = 1;}		//test for ack
	data = I2C_read();
												//no ack if we only want one byte
	I2C_stop();									//all done
	while ( SSPCON2bits.PEN );					//wait for stop bit to finish
	if ( PIR2bits.BCLIF ){no_ack = 1;}			//test for bus collision
